...
router.get('/', async (req, res) => {
    try {
        const strategies = await prisma_1.prisma.strategy.findMany({
...
router.post('/', [
    (0, express_validator_1.body)('name').notEmpty().trim().isLength({ min: 1, max: 100 }),
    ...
    try {
        ...
        const existingStrategy = await prisma_1.prisma.strategy.findFirst({
            where: {
                name: name.trim(),
                userId: req.user.id
            }
        ...
        const strategy = await prisma_1.prisma.strategy.create({
            data: {
                name: name.trim(),
...
router.put('/:id', [
    ...
    try {
        ...
        if (name && name.trim() !== existingStrategy.name) {
            const duplicateStrategy = await prisma_1.prisma.strategy.findFirst({
                where: {
                    name: name.trim(),
                    userId: req.user.id,
                    id: { not: id }
        ...
        const strategy = await prisma_1.prisma.strategy.update({
            where: { id },
            data: updateData,
            include: {
                trades: {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await prisma_1.prisma.strategy.delete({
            where: { id }
        });
        res.json({
